...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        self.graph_feature = args.use_graph_feature
        # GC
        ...
        for i in range(num_pooling):
            ...
            if not concat:
                ...
                assign_input_dim =2 * hidden_dim + embedding_dim
        ...
        for m in self.modules():
            if isinstance(m, GraphConv):
                ...
                if m.bias is not None:
                    m.bias.data = init.constant_(m.bias.data, 0.0)
        # todo: check
        self.conv_first_after_pool.apply(self.weights_init)
        self.conv_block_after_pool.apply(self.weights_init)
        self.conv_last_after_pool.apply(self.weights_init)
        self.assign_conv_block_modules.apply(self.weights_init)
        self.assign_conv_first_modules.apply(self.weights_init)
        self.assign_conv_last_modules.apply(self.weights_init)
        self.assign_pred_modules.apply(self.weights_init)
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        for i in range(self.num_pooling):
            ...
            assign_tensor = self.gcn_forward(x_a, adj,
                    self.assign_conv_first_modules[i], self.assign_conv_block_modules[i], self.assign_conv_last_modules[i],
                    embedding_mask)

            # [batch_size x num_nodes x next_lvl_num_nodes]

            assign_tensor = nn.Softmax(dim=-1)(self.assign_pred_modules[i](assign_tensor))
            if embedding_mask is not None:
                assign_tensor = assign_tensor * embedding_mask
            self.assign_tensor_list.append(assign_tensor)
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1, k_sample = 3000):
        ''' 
        Args:
        ...
        # todo fix bug for 2 pool layer, change assign-tensor
        if self.linkpred:

            # for the first pool layer
            max_num_nodes = adj.size()[1]
            # hack: since the first layer is so large space, we random sample a fix number
            samples = []
            pred_adjs = []
            adjs = []
            batch_num_nodes[batch_num_nodes<k_sample] = k_sample
            for idx, bs in enumerate(batch_num_nodes):
                # bs = torch.max(torch.Tensor(k_sample).long().cuda(), bs)
                sample = torch.randperm(bs)[:k_sample]
                pred_adj =  self.assign_tensor_list[0][idx] @ torch.transpose(self.assign_tensor_list[0][idx], 0, 1)
                pred_adj = pred_adj[ sample,:]
                pred_adj = pred_adj[:,sample]
                pred_adjs.append(pred_adj)
                sub_adj = adj[idx]
                sub_adj = sub_adj[sample,:]
                sub_adj = sub_adj[:,sample]
                adjs.append(sub_adj)
            pred_adjs = torch.stack(pred_adjs, 0)
            adjs = torch.stack(adjs, 0)
            # pred_adj = self.assign_tensor_list[0] @ torch.transpose(self.assign_tensor_list[0], 1, 2)
            # pred_adj = pred_adj
            # tmp = pred_adj0
            # pred_adj = pred_adj0
            ...
            # pred_adj = torch.min(pred_adj0, torch.Tensor(1).cuda())
            ...
            link_loss = - adjs * torch.log(pred_adjs+eps) - (1- adjs) * torch.log(1-pred_adjs+eps)
            link_loss = torch.sum(link_loss)/adjs.numel()
            ...
            #
            # self.link_loss = torch.sum(link_loss) / float(num_entries)
            # #print('linkloss: ', self.link_loss)
            # self.ent_loss = (-pred_adjs * torch.log(pred_adjs + EPS)).sum(dim=-1).mean()
            #
            # if len(self.assign_tensor_list)>1:
            #     # more than one pool
            #     adj = pred_adjs
            #     for assign_tensor in self.assign_tensor_list[1:]:
            #         pred_adj = assign_tensor @ torch.transpose(assign_tensor,1,2)
            #         link_loss = -adj * torch.log(pred_adj+eps) - (1-adj) * torch.log(1-pred_adj+eps)
            #         link_loss = torch.sum(link_loss)/adj.numel()
            #         self.link_loss += link_loss
            #         self.ent_loss += (-pred_adj * torch.log(pred_adj + EPS)).sum(dim=-1).mean()
